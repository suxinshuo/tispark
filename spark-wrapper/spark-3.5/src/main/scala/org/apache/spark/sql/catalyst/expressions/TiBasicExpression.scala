...
object TiBasicExpression {
  ...
  def convertToTiExpr(expr: Expression): Option[TiExpression] =
    expr match {
      ...
      // Spark 3.4+ removed PromotePrecision (SPARK-39316): decimal binary arithmetic
      // no longer wraps its casts in PromotePrecision, so the cast to the common
      // decimal type now appears directly. Preserve the original push-down behavior
      // by stripping the decimal Cast and tagging the inner expression's type.
      case Cast(BasicExpression(expr), dec: DecimalType, _, _) =>
        expr.setDataType(TiConverter.fromSparkType(dec))
        Some(expr)
